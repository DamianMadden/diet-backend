...
{
    public class RecipeIngestion(IEnumerable<IIngestionSource> sources, DietDbContext dietDb,
        ILogger<RecipeIngestion> logger) : BackgroundService
    {
        ...
        protected async override Task ExecuteAsync(CancellationToken cancel)
        {
            ...
            var latestIngestionRecords = await dietDb.IngestionRecords.AsNoTracking()
            ...
            foreach (var source in sources)
            {
                ...
                var latestRecord = latestIngestionRecords.Where(rec => rec!.Source == sourceKey).FirstOrDefault();
